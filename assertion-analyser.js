...
function replacer(str) { /* replace objects with a symbol ( __#n) */
  let obj;
  let cnt = 0;
  let data = [];
...
function splitter(str) { /* split on commas, then restore the objects */
...
function assertionAnalyser(body) {  
  ...
  assertions = methods.map(function(m, i) {
    return { method: m, args: splitter(bodies[i]) }; 
